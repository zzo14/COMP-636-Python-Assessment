...
#Use classes to encapsulate Driver and Run
class Driver:
    def __init__(self, driver_id, first_name, last_name, category, age, caregiver_id=None):
...
class Run:
    def __init__(self, run_id, course, driver_id, time=None, cones_hit=None, wd=None, Run_Total=None):
        self.run_id = run_id
        self.course = course
        self.driver_id = driver_id
        self.time = time
        self.cones_hit = cones_hit
        self.wd = wd
        self.Run_Total = Run_Total

# Convert db_drivers dictionary to a list of Driver objects
drivers_list = []
for driver_id, driver_data in brmm_data.db_drivers.items():
    drivers_list.append(Driver(driver_id, *driver_data))

# Convert db_runs dictionary to a list of Run objects
runs_list = []
for run_id, run_data in brmm_data.db_runs.items():
    runs_list.append(Run(run_id, *run_data))
...
def list_juniors_by_age():
    ...
    for driver in drivers_list:
        if driver.category == 'J':
            caregiver_name = ""
            if 12 <= driver.age <= 16 and driver.caregiver_id is not None:
                caregiver = next((c for c in drivers_list if c.driver_id == driver.caregiver_id), None)
                if caregiver:
                    caregiver_name = f"{caregiver.first_name} {caregiver.last_name}"
            display_list.append((driver.driver_id,
                                f"{driver.first_name} {driver.last_name}",
                                driver.age,
                                caregiver_name))
            
    sorted_display_list = sorted(display_list, key=lambda entry: entry[2]) # sort display list by age
...
def calculate_run_time(run):
    # Calculate total time for a run
    if run.time is None:
        return None
    return run.time + float(run.cones_hit or 0) * 5 + run.wd * 10 # calculate total run time with cones hits and wd
...
def list_runs():
    ...
    for run in runs_list:
        driver = next((d for d in drivers_list if d.driver_id == run.driver_id), None)
        if driver is None:
            continue

        driver_name = f"{run.driver_id} - {driver.first_name} {driver.last_name}"
        display_list.append((run.run_id,
                             run.course,
                             driver_name,
                             run.time if isinstance(run.time, (int, float)) else None,
                             run.cones_hit,
                             'YES' if run.wd == 1 else 'NO',
                             calculate_run_time(run)))

    sorted_display_list = sorted(display_list, key=lambda x: (x[1], x[6] is None, x[6]))
...
def edit_run_results():
    ...
    while True:
        ...
        try:
            ...
            print(f"\nError: Invalid input for Run ID '{run_id_input}'. Please enter a valid integer.")
            continue

        run = next((r for r in runs_list if r.run_id == run_id), None)

        if run is not None:
            print(f"\nUpdating Run ID {run_id}...")

            time_flag = True
            time_enter = get_numeric_input("\nPlease enter the time (or Press Enter to keep existing time): ")
            if not time_enter:
                if run.time is None:
                    time_flag = False
            ...
            if time_flag:
                cones_hits = get_numeric_input("\nPlease enter the number of cones hits (or Press Enter for 0): ", is_integer=True)
                print("\nCones Hits record entered.")
                wd_enter = input("\nEnter 'WD/wd' for wrong direction (or press Enter for 0): ")
                wd_enter = 1 if wd_enter.upper() == 'WD' else 0
                print("\nWrong Direction record entered.")

                if time_enter != "":
                    run.time = time_enter
                    run.cones_hit = cones_hits
                    run.wd = wd_enter
                else:
...
def calculate_total_time():
    ...
    total_time = {} 
    for run in runs_list:
        individual_run_times = calculate_run_time(run)
        if run.time is not None:
            if run.driver_id not in total_time:
                total_time[run.driver_id] = [individual_run_times]
            else:
                total_time[run.driver_id].append(individual_run_times)
...
def display_final_results():
    ...
    display_list = []
    for driver in drivers_list:
        driver_name = f"{driver.first_name} {driver.last_name} (J)" if driver.category == 'J' else f"{driver.first_name} {driver.last_name}"
        display_list.append((driver.driver_id,
                             driver_name,
                             total_time.get(driver.driver_id, 'HAG')))#have no recorded show as "HAG"
    ...
    input("\nPress Enter to continue.")     # Pauses the code to allow the user to see the outpu


#6. Cone graph
def display_cone_graph():
    # List any drivers who hit cones, along with their toteal numbers of cones as a repeating symbol or character.
    ...
    for run in runs_list:
        cone_hit = run.cones_hit
        if cone_hit is not None and cone_hit > 0:
            driver_id = run.driver_id
            cones_data[driver_id] = cone_hit if driver_id not in cones_data else cones_data[driver_id] + cone_hit
    for id, cones_hit in cones_data.items():
        driver = [driver for driver in drivers_list if driver.driver_id == id][0]
        driver_name = f"{driver.driver_id} - {driver.first_name} {driver.last_name}"
        display_list.append((driver_name, " Δ" * cones_hit))
